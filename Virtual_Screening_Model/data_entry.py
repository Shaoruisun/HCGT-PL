...
data_dir='/DUD-E'
data=PDBbindDataset(ids="%s/out_id_train.npy"%(data_dir),protein_ligand_1="%s/out_train.bin"%(data_dir),target="%s/out_target_train.npy"%(data_dir))
train_inds, val_inds =data.train_and_test_split(valnum=10000, seed=1200)#2023#1000
print('训练集个数',len(train_inds))
print('测试集个数',len(val_inds))

def collate(data):
	pdbids, graphs_0, graphs_1,label= map(list, zip(*data))
...
def get_dataset_by_type_train(args, is_train=False):
    type2data = {
        'PDBbind2016': PDBbindDataset(ids=data.pdbids[train_inds],
							protein_ligand_1=np.array(data.graphs_1)[train_inds],target=data.label[train_inds]
							)}
...
def get_dataset_by_type_val(args, is_train=False):
    type2data = {
        'PDBbind2016': PDBbindDataset(ids=data.pdbids[val_inds],
							protein_ligand_1=np.array(data.graphs_1)[val_inds],target=data.label[val_inds]
...
def select_train_loader(args):
    ...
    train_loader = DataLoader(train_dataset,args.batch_size, shuffle=True, num_workers=16,collate_fn=collate,drop_last=False)#, pin_memory=True, drop_last=False)
    return train_loader
...
def select_eval_loader(args):
    ...
    val_loader = DataLoader(eval_dataset,args.batch_size, shuffle=False, num_workers=16,collate_fn=collate,drop_last=False)#, pin_memory=True, drop_last=False)
    return val_loader
