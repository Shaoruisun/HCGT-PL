...
class PDBbindDataset(Dataset):
    def __init__(self,
        ...
        if isinstance(protein_ligand_2, np.ndarray) or isinstance(protein_ligand_2, tuple) or isinstance(protein_ligand_2, list):
            if isinstance(protein_ligand_2[0], dgl.DGLGraph):
                self.graphs_2 = protein_ligand_2
            else:
                raise ValueError('the variable "ligs" should be a set of (or a file to store) dgl.DGLGraph objects.')
        else:
            try:
                self.graphs_2, _ = load_graphs(protein_ligand_2)
            except:
                raise ValueError('the variable "ligs" should be a set of (or a file to store) dgl.DGLGraph objects.')
        
        #self.graphs= list(self.graphs)
        index_label_file='/data/INDEX_refined_data.2016'
        contents = []
    ...
    def __getitem__(self, idx):
        ...
        return self.pdbids[idx],self.graphs_1[idx],self.graphs_2[idx],self.label
